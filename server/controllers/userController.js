...
exports.getEntities = async (req,res) => {
  ...
  var conditions = { }
  try {
    const result = await  User.paginate(conditions, options);    
    let data = { data: result , message: "user Listing" }
    res.status(200).json(data);
  } catch (err) {
    console.log(err)
    res.status(500).json(err);
...
exports.updateEntityById = async (req,res) => {
    ...
    try {
      const result = await User.updateOne(conditions, update).exec();
